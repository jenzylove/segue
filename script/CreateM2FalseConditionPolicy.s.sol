...
contract CreateM2FalseConditionPolicy {
    IFalsePolicyVm internal constant VM = IFalsePolicyVm(address(uint160(uint256(keccak256("hevm cheat code")))));
    ...
    function run() external returns (uint256 policyId) {
        ...
        uint256 ttlSeconds = VM.envUint("M2_FALSE_POLICY_TTL_SECONDS");

        if (
            amount == 0 || expectedPolicyId == 0 || bufferBps == 0 || bufferBps >= 5_000 || maxDeviationBps == 0
                || maxDeviationBps >= BPS || ttlSeconds < 600 || ttlSeconds > 86_400
        ...
        // False-condition and stale-feed probes return/revert before the balance
        // check. A lossy round trip must not require topping up just to read them.
        ...
        policyId = abi.decode(
            M2Attribution.callWithSuffix(
                address(vault), abi.encodeCall(StockPolicyVault.createPolicy, (steps, amount)), attribution
            ),
            (uint256)
        );
        VM.stopBroadcast();
